...
/* begin: [pengruofeng 2015-5-29 ]  add route mac to redirect url  */
...
static int urmtmng_get_dev_mac_eoc(char *eMac, char *dev_name)
{	
	struct ifreq ifr;
	int    skfd = 0;
	if ((skfd = socket(AF_INET, SOCK_DGRAM, 0)) < 0)	
	{		
		debug(LOG_INFO, "get MAC address:open socket error\n");	
		return -1;	
	}	
	if(strncmp(dev_name, "eth2.1", 3) == 0)	
	{
		strncpy(ifr.ifr_name, dev_name, 16);	
		if (ioctl(skfd, SIOCGIFHWADDR, &ifr) < 0) 	
		{			
			debug(LOG_INFO, "get MAC address: ioctl error, cmd = 0x%x, dev_name=%s\n", SIOCGIFHWADDR,dev_name);	
			close(skfd);		
			return -1;		
		}				
		memcpy(eMac, ifr.ifr_hwaddr.sa_data, 6);	
	}
...
http_callback_404(httpd *webserver, request *r)
{
	...
	/* begin: [pengruofeng 2015-5-29 ]  add route mac to redirect url  */
	unsigned char rmac[6] = {0};
	char s_rmac[18];
	int rtn = 0;

	memset(s_rmac, '\0', sizeof(s_rmac));
	rtn = urmtmng_get_dev_mac_eoc(rmac, "eth2.1"); 
	if(rtn == 0) {
		debug(LOG_INFO, "get mac sucess %.2X:%.2X:%.2X:%.2X:%.2X:%.2X , %s, %d\r\n", rmac[0], rmac[1],
                                rmac[2], rmac[3], rmac[4], rmac[5], __FUNCTION__, __LINE__);
		sprintf(s_rmac, "%02x:%02x:%02x:%02x:%02x:%02x",
					rmac[0], rmac[1], rmac[2], rmac[3], rmac[4], rmac[5]);
	}
	else {
		debug(LOG_INFO, "Get router mac failed, %s, %d", __FUNCTION__, __LINE__);
		return;
	}
	...
	if (!is_online()) {
		...
		if (!(mac = arp_get(r->clientAddr))) {
			...
			/*safe_asprintf(&urlFragment, "%sgw_address=%s&gw_port=%d&gw_id=%s&mac=%s&url=%s",
				...
				mac,
				url);*/
			safe_asprintf(&urlFragment, "%sgw_address=%s&gw_port=%d&gw_id=%s&umac=%s&rmac=%s&url=%s",
				auth_server->authserv_login_script_path_fragment,
				config->gw_address,
				config->gw_port, 
				config->gw_id,
