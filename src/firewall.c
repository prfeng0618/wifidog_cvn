...
fw_sync_with_authserver(void)
{
    ...
    for (p1 = p2 = client_get_first_client(); NULL != p1; p1 = p2) {
        ...
        if (!(p1 = client_list_find(ip, mac))) {
        	...
        	debug(LOG_INFO, "Checking client %s for timeout: Login time %ld Last updated %ld (%ld seconds ago), timeout delay %ld seconds, current time %ld, ",
                        p1->ip, p1->counters.login_time, p1->counters.last_updated, current_time-p1->counters.last_updated, config->checkinterval * config->clienttimeout, current_time);
        	...
        	} else if(p1->counters.login_time+ config->kickclienttime
				<= current_time){
				
                /* Timing out user */
                debug(LOG_INFO, "%s - rule : kick client for more than %ld seconds, removing client and denying in firewall",
                        p1->ip, config->kickclienttime);
                fw_deny(p1->ip, p1->mac, p1->fw_connection_state);
                client_list_delete(p1);
