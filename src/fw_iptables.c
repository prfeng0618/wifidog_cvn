...
static int SET_RULE_RETRY_COUNT=10;
...
iptables_do_command(const char *format, ...)
{
	...
	/* BEGIN: added by wuhd for New Portal Certification, 20140911 */
	/* 防火墙规则一旦下发失败，尝试重新下发，最多5 次 */
	/*
	int count = 0;
	while (1) {
			...
			if (rc!=0) {
					...
					if (fw_quiet == 0) {
							...
							count++;
							if (count > SET_RULE_RETRY_COUNT) {
									break;
							}
							printf("iptables command failed(%d): %s\n", rc, cmd);
							printf("One more try\n");
							sleep(3);
							continue;
					}
